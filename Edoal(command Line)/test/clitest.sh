#!/bin/bash
#
# This long script runs commands from the command line
#
# It is highly dependent on logback and my (JE) own logback.xml
#

export CWD=`pwd`
export CP=$CWD/lib/procalign.jar:$CWD/lib/slf4j/logback-core-1.2.3.jar:$CWD/lib/slf4j/logback-classic-1.2.3.jar:$CWD
export RESDIR='/tmp/clitest'
...
echo -e "\t\tTHE -z OPTIONS RELY ON LOGBACK BEING PROPERLY DEFINED"
echo -e "\t\t *** Testing Procalign ***"
...
#-------------------
echo -e "\t-r,--renderer <CLASS>"
java -cp $CP fr.inrialpes.exmo.align.cli.Procalign file://$CWD/examples/rdf/onto1.owl file://$CWD/examples/rdf/onto2.owl -r fr.inrialpes.exmo.align.impl.renderer.OWLAxiomsRendererVisitor | sed "s/comment>time: [0-9]*/comment>time: 0/" > $RESDIR/proc-r.rdf
java -cp $CP fr.inrialpes.exmo.align.cli.Procalign file://$CWD/examples/rdf/onto1.owl file://$CWD/examples/rdf/onto2.owl --renderer fr.inrialpes.exmo.align.impl.renderer.OWLAxiomsRendererVisitor | sed "s/comment>time: [0-9]*/comment>time: 0/" > $RESDIR/proc-render.rdf
if [ -s $RESDIR/proc-r.rdf ]; then diff $RESDIR/proc-r.rdf $RESDIR/proc-render.rdf; else echo error with PROC-RENDER1; fi

#-------------------
echo -e "\t-P,--params <FILE>"
...
echo -e "\t\t *** Testing EvalAlign ***"

#-------------------
echo -e "\t-d(<L>), --debug (<L>) DEPRECATED"
...
#-------------------
echo -e "\t-u,--urlprefix <URI>  --uriprefix"
...
#if [ ! -s $RESDIR/smalltest/outtestdir2/ontodiff.txt ]; then echo error with GEN-OUT2; fi
if [ -s $RESDIR/smalltest/outtestdir1/refalign.rdf ]; then diff $RESDIR/smalltest/outtestdir1/refalign.rdf $RESDIR/smalltest/outtestdir2/refalign.rdf; else echo error with GEN-OUT3; fi
...
if [ -s $RESDIR/smalltest/outtestdir3/onto.rdf ]; then diff $RESDIR/smalltest/outtestdir1/refalign.rdf $RESDIR/smalltest/outtestdir3/refalign.rdf >$RESDIR/smalltest/outtestdir3/ontodiff.txt; else echo error with GEN-VAR1; fi
if [ ! -s $RESDIR/smalltest/outtestdir3/ontodiff.txt ]; then echo error with GEN-VAR2; fi

#-------------------
echo -e "\t-a,--alignname <FILE> "
echo -e "\t-o,--output <FILE>"
...
if [ -s $RESDIR/outtestdir4/biblio.owl ]; then diff $RESDIR/outtestdir5/biblio.owl $RESDIR/outtestdir4/biblio.owl; else echo error with GEN-AL1; fi
if [ -s $RESDIR/outtestdir4/myalign.rdf ]; then diff $RESDIR/outtestdir5/myalign.rdf $RESDIR/outtestdir4/myalign.rdf; else echo error with GEN-AL2; fi
diff $RESDIR/outtestdir5/myalign.rdf $RESDIR/smalltest/outtestdir1/refalign.rdf > $RESDIR/outtestdir5/gendiff.txt
if [ ! -s $RESDIR/outtestdir5/gendiff.txt ]; then echo error with GEN-AL3; fi

#-------------------
echo -e "\t-t,--testset <CLASS>"
...
# JE 2017-07
#sed -i'' "s;<location>http://www.example.org/</location>;<location>file://$RESDIR/smalltest/${i}onto.rdf</location>;" $RESDIR/smalltest/${i}refalign.rdf
sed -i'' "s;<location>http://www.example.org/</location>;<location>file://$RESDIR/smalltest/gentestempty1/onto1.rdf</location>;" $RESDIR/smalltest/${i}refalign.rdf
...
sed -i'' "s;<td property=\"ns0\:time\">[^<]*</td>;;g" $RESDIR/smalltest/gentestempty1/streq1.html
sed -i'' "s;<td property=\"ns0\:time\">[^<]*</td>;;g" $RESDIR/smalltest/gentestempty1/streq2.html
...
#-------------------
echo -e "\t-Dn=v"
echo -e "\t(same as Procalign; useless: no parameters involved so far)"
...
#-------------------
echo -e "\t-t,--type <TYPE>         Output TYPE (html|xml|tex|ascii|triangle)"
...
#-------------------
echo -e "\t-t,--type <TYPE>         Output TYPE (tex|html) / only tex available in fact"
...
#-------------------
echo -e "\t-t,--type <TYPE>         Output TYPE (tsv|tex|html(|xml))"
...
echo -e "\t\t *** Testing AlignmentService ***"
echo -e "\tTHIS WILL ONLY WORK WITH A RUNNING MYSQL SET FOR THE SERVER"
echo -e "\tTHIS IS ALSO VERY SENSITIVE TO TIMEOUTS"
...
#-------------------
echo -e "\t-A,--jade <PORT>"
echo -e "\tOnly works with increased PermGenSize"
...
kill -TERM $!
java -XX:PermSize=128m -cp $CP:lib/alignsvc.jar fr.inrialpes.exmo.align.service.AlignmentService --jade 5556 &> $RESDIR/err.txt &
sleep 15;
...
# From here -x is used until we implement ssl connections
#-------------------
echo -e "\t-W,--wsdl <PORT>"
java -cp $CP:lib/alignsvc.jar fr.inrialpes.exmo.align.service.AlignmentService -x -W 5557 &> $RESDIR/err.txt &
sleep 15;
kill -TERM $!
if [ -s $RESDIR/err.txt ]; then echo error with ASERV-WSDL1; fi
java -cp $CP:lib/alignsvc.jar fr.inrialpes.exmo.align.service.AlignmentService -x --wsdl 5558 &> $RESDIR/err.txt &
sleep 15;
kill -TERM $!
...
#-------------------
echo -e "\t-H,--http <PORT>"
java -cp $CP:lib/alignsvc.jar fr.inrialpes.exmo.align.service.AlignmentService -x -H 5559 &> $RESDIR/err.txt &
sleep 15;
kill -TERM $!
if [ -s $RESDIR/err.txt ]; then cat $RESDIR/err.txt; echo error with ASERV-HTTP1; fi
java -cp $CP:lib/alignsvc.jar fr.inrialpes.exmo.align.service.AlignmentService -x --http 5551 &> $RESDIR/err.txt &
sleep 15;
kill -TERM $!
if [ -s $RESDIR/err.txt ]; then cat $RESDIR/err.txt; echo error with ASERV-HTTP2; fi

#-------------------
echo -e "\t-X,--jxta <PORT>"
java -cp $CP:lib/alignsvc.jar fr.inrialpes.exmo.align.service.AlignmentService -x -X 5552 &> $RESDIR/err.txt &
sleep 15;
kill -TERM $!
if [ -s $RESDIR/err.txt ]; then cat $RESDIR/err.txt; echo error with ASERV-JXTA1; fi
java -cp $CP:lib/alignsvc.jar fr.inrialpes.exmo.align.service.AlignmentService -x --jxta 5553 &> $RESDIR/err.txt &
sleep 15;
kill -TERM $!
if [ -s $RESDIR/err.txt ]; then cat $RESDIR/err.txt; echo error with ASERV-JXTA2; fi

#-------------------
echo -e "\t-O,--oyster"
echo -e "\t\tTO BE DEPRECATED"
...
#-------------------
echo -e "\t-S,--host <HOSTNAME>"
java -cp $CP:lib/alignsvc.jar fr.inrialpes.exmo.align.service.AlignmentService -x -S aserv.inria.fr &> $RESDIR/err.txt &
sleep 15;
kill -TERM $!
if [ -s $RESDIR/err.txt ]; then echo error with ASERV-HOST1; fi
java -cp $CP:lib/alignsvc.jar fr.inrialpes.exmo.align.service.AlignmentService -x --host aserv.inria.fr &> $RESDIR/err.txt &
sleep 15;
kill -TERM $!
...
#-------------------
echo -e "\t-u,--uriprefix <URI>"
java -cp $CP:lib/alignsvc.jar fr.inrialpes.exmo.align.service.AlignmentService -x -u http://www.example.org &> $RESDIR/err.txt &
sleep 15;
kill -TERM $!
if [ -s $RESDIR/err.txt ]; then echo error with ASERV-URIP1; fi
java -cp $CP:lib/alignsvc.jar fr.inrialpes.exmo.align.service.AlignmentService -x --uriprefix http://www.example.org &> $RESDIR/err.txt &
sleep 15;
kill -TERM $!
...
if [ ! -s $RESDIR/err.txt ]; then echo error with ASERV-DBHOST1; else grep "Cannot connect to database" $RESDIR/err.txt > $RESDIR/dberr.txt; fi
...
if [ ! -s $RESDIR/err.txt ]; then echo error with ASERV-DBHOST3; else grep "Cannot connect to database" $RESDIR/err.txt > $RESDIR/dberr.txt; fi
if [ ! -s $RESDIR/dberr.txt ]; then echo error with ASERV-DBHOST4; fi

#-------------------
echo -e "\t-s,--dbmsport <PORT>"
java -Dlog.level=ERROR -cp $CP:lib/alignsvc.jar fr.inrialpes.exmo.align.service.AlignmentService -s 5555 &> $RESDIR/err.txt
if [ ! -s $RESDIR/err.txt ]; then echo error with ASERV-DBPORT1; else grep "Cannot connect to database" $RESDIR/err.txt > $RESDIR/dberr.txt; fi
...
if [ ! -s $RESDIR/err.txt ]; then echo error with ASERV-DBPORT3; else grep "Cannot connect to database" $RESDIR/err.txt > $RESDIR/dberr.txt; fi
if [ ! -s $RESDIR/dberr.txt ]; then echo error with ASERV-DBPORT4; fi

#-------------------
echo -e "\t-b,--dbmsbase <BASE>"
java -Dlog.level=ERROR -cp $CP:lib/alignsvc.jar fr.inrialpes.exmo.align.service.AlignmentService -b myAlignDB &> $RESDIR/err.txt
if [ ! -s $RESDIR/err.txt ]; then echo error with ASERV-DBDB1; else grep "Cannot connect to database" $RESDIR/err.txt > $RESDIR/dberr.txt; fi
...
if [ ! -s $RESDIR/err.txt ]; then echo error with ASERV-DBDB3; else grep "Cannot connect to database" $RESDIR/err.txt > $RESDIR/dberr.txt; fi
if [ ! -s $RESDIR/dberr.txt ]; then echo error with ASERV-DBDB4; fi

#-------------------
echo -e "\t-l,--dbmsuser <USER>"
java -Dlog.level=ERROR -cp $CP:lib/alignsvc.jar fr.inrialpes.exmo.align.service.AlignmentService -l scott &> $RESDIR/err.txt
if [ ! -s $RESDIR/err.txt ]; then echo error with ASERV-DBUSR1; else grep "Cannot connect to database" $RESDIR/err.txt > $RESDIR/dberr.txt; fi
...
if [ ! -s $RESDIR/err.txt ]; then echo error with ASERV-DBUSR3; else grep "Cannot connect to database" $RESDIR/err.txt > $RESDIR/dberr.txt; fi
if [ ! -s $RESDIR/dberr.txt ]; then echo error with ASERV-DBUSR4; fi

#-------------------
echo -e "\t-p,--dbmspass <PASS>"
java -Dlog.level=ERROR -cp $CP:lib/alignsvc.jar fr.inrialpes.exmo.align.service.AlignmentService -p tiger &> $RESDIR/err.txt
if [ ! -s $RESDIR/err.txt ]; then echo error with ASERV-DBPAS1; else grep "Cannot connect to database" $RESDIR/err.txt > $RESDIR/dberr.txt; fi
...
if [ ! -s $RESDIR/err.txt ]; then echo error with ASERV-DBPAS3; else grep "Cannot connect to database" $RESDIR/err.txt > $RESDIR/dberr.txt; fi
